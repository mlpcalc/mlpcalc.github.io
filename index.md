...
<script>
  ...
  const maindata = gamedata["The Crowning Achievement"]
  const images = ["pouchpic","coinpic"];
  const vals = maindata["upgrades"];
  var curarmor = [vals[0][0],0]; //current value, index in vals
  var curhelmet = [vals[0][0],0];
  ...
  function setHelpers(){
    for (let i=0;i<stages.length;i++){
	  ...
	  stages[i]['coinbonus'] = 0
	}
    if (document.getElementById("help0").checked){
	  stages[0]['helpersdmg'] += 2
	  stages[1]['helpersdmg'] += 5
	  stages[1]['coinbonus'] += 50
	  stages[2]['helpersdmg'] += 20
	  stages[2]['coinbonus'] += 75
	}
	...
	if (document.getElementById("help3").checked){
	  stages[3]['helpersdmg'] += 45
	  stages[3]['coinbonus'] += 100
	  stages[4]['helpersdmg'] += 65
	  stages[4]['coinbonus'] += 125
	}
	if (document.getElementById("help4").checked){
	  stages[3]['helpersdmg'] += 45
	  stages[3]['coinbonus'] += 100
	  stages[4]['helpersdmg'] += 65
	  stages[4]['coinbonus'] += 125
	}
	if (document.getElementById("help5").checked){
	  stages[3]['helpersdmg'] += 45
	  stages[3]['coinbonus'] += 100
	  stages[4]['helpersdmg'] += 65
	  stages[4]['coinbonus'] += 125
	}
	if (document.getElementById("help6").checked){
	  stages[5]['helpersdmg'] += 100
	  stages[5]['coinbonus'] += 150
	  stages[6]['helpersdmg'] += 150
	  stages[6]['coinbonus'] += 175
	  stages[7]['helpersdmg'] += 100
	  stages[7]['coinbonus'] += 175
	}
	if (document.getElementById("help7").checked){
	  stages[5]['helpersdmg'] += 100
	  stages[5]['coinbonus'] += 150
	  stages[6]['helpersdmg'] += 150
	  stages[6]['coinbonus'] += 175
	...
	if (document.getElementById("help8").checked){
	  stages[5]['helpersdmg'] += 100
	  stages[5]['coinbonus'] += 150
	  stages[6]['helpersdmg'] += 150
	  stages[6]['coinbonus'] += 175
  ...
  function calctime(){
	...
	for (let i=curstage; i < stages.length-1; i++){
	  ...
	  for (let j=1; j<egems.length;j++){
	    tr.innerHTML += '<td width="120"><img name="gempic" src="" alt="Gems" />'+egems[j] + ': ' + formattime(gemtimes[j]) + '</td>';
	  }
	  tbdy.appendChild(tr);
...
<h1>The Crowning Achievement calculator</h1>
...
<input id="curarmor" type="number" value="60" min="1" max="88" style="width:80px"/><br>
<input id="curhelmet" type="number" value="60" min="1" max="88" style="width:80px"/>

   <p id="helpers"><b>Helpers:</b><br>
   <table><tr>
   <td><input id="help0" type="checkbox" onchange=""> <span id="">Future Fluttershy</span></td>
   <td><input id="help1" type="checkbox" onchange=""> <span id="">Future Applejack</span></td>
   <td><input id="help2" type="checkbox" onchange=""> <span id="">Future Rainbow Dash</span></td>
   </tr><tr>
   <td><input id="help3" type="checkbox" onchange=""> <span id="">Future Rarity</span></td>
   <td><input id="help4" type="checkbox" onchange=""> <span id="">Future Pinkie Pie</span></td>
   <td><input id="help5" type="checkbox" onchange=""> <span id="">Future Apple Bloom</span></td>
   </tr><tr>
   <td><input id="help6" type="checkbox" onchange=""> <span id="">Future Starlight Glimmer</span></td>
   <td><input id="help7" type="checkbox" onchange=""> <span id="">Future Sunburst</span></td>
...
<br>   
 Current stage: <select id="curstage">
    <option value="0">1. Mrs. Cup Cake (100, 1)</option>
	<option value="1">2. Spitfire (800, 2)</option>
	<option value="2">3. Moon Dancer (1100, 3)</option>
	<option value="3">4. Fancypants (3200, 9)</option>
	<option value="4" selected>5. Prince Blueblood (8000, 24)</option>
